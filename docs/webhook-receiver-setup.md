...
התשובה כוללת `received`, `appended`, `skipped` ו-`sheet`, ואם עמודת
`wa_message_id` חסרה גם `warning`. שלושת המספרים מסבירים כל תגובה בלוג בלי
לנחש, ו-`sheet` הוא מה שמוכיח שהכתיבה נחתה בלשונית הנכונה ולא בלשונית שנוצרה
בטעות — כשל שקט שקל לפספס.
...
## `diagnose.cmd` — לוג של בקשה אחת, בחלון אחד

להחזיק `wrangler tail` בחלון אחד ובקשה בחלון שני מאבד את הפלט ברגע שאחד מהם
נסגר. `diagnose.cmd` עושה את שניהם ומדפיס את התוצאה:

```cmd
diagnose.cmd
```

הוא מפעיל את ה-tail לקובץ, שולח **בקשה אחת בלי חתימה** (שאמורה להידחות),
ומדפיס את מה שה-Worker רשם.

הבקשה נדחית בכוונה. המטרה היא לראות שהדחייה **מגיעה ללוג** — כלומר שהלוג
אמין — לפני שמשתמשים בו לאבחן משהו אחר.

| מה יצא | מה זה אומר |
|---|---|
| `401 signature rejected: ...` | הלוג אמין. שקט בלחיצה על Send אצל Meta פירושו ש-Meta לא מגיעה ל-Worker |
| אין שורה כזו | הלוג לא מראה כלום, וכל אבחון שנעשה איתו היה עיוור |


## הרצה חיה ראשונה — 21.9.2026

הצינור נסגר מקצה לקצה. מסירה של Meta נכתבה לגיליון:

```
2026-09-21T12:19:02.733Z | ua=facebookexternalua
log: wrote 1 lead(s)
```

לפני זה נמצאו שלוש תקלות, **כולן קונפיג ואף אחת לא בקוד**. כל אחת הסתירה את
הבאה אחריה, ולכן הן התגלו בזו אחר זו ולא יחד.

| ערך | מה היה בו | מה הלוג אמר |
|---|---|---|
| `SHEETS_WEBAPP_URL` | פקודת טרמינל שהודבקה לפרומפט במקום ה-URL | `sink write failed Invalid URL: cd worker && ...` |
| `SHEETS_TOKEN` | לא תאם ל-`TOKEN` בגרסה הפרוסה | `sheets sink refused: unauthorized` |
| `APP_SECRET` | ה-`VERIFY_TOKEN`, לא ה-App Secret | `401 signature rejected: header present` |

### שלוש מסקנות תפעוליות

**`wrangler secret list` מראה שמות, לא ערכים.** ארבעה סודות "מוגדרים" ושניים
מהם זבל נראה בדיוק כמו מערכת תקינה. ערך שגוי מתגלה רק כשמישהו משתמש בו.

**`APP_SECRET` ו-`VERIFY_TOKEN` מתחלפים בקלות.** `VERIFY_TOKEN` נבחר על ידי
המשתמש ולכן נזכר; `APP_SECRET` מיוצר על ידי Meta, תמיד 32 תווי hex קטנים,
ואי אפשר לקבוע אותו — רק Reset. ערך שאינו 32 hex הוא בוודאות לא App Secret.

**`probe.mjs` לא בודק את `APP_SECRET`.** הוא חותם עם מה שנותנים לו, ולכן הוא
עובר תמיד כשנותנים לו את הערך שב-Worker — גם כשהערך הזה שגוי. הוא מוכיח את
הקטע Worker→גיליון בלבד. רק מסירה אמיתית מ-Meta מכריעה על החתימה.

### ה-404 המקרי

חלק מהמשלוחים חוזרים `sheets sink HTTP 404` עם דף שגיאה של גוגל, בזמן
שהשורה **כן נכתבה**. `/exec` מחזיר 302, `doPost` כבר רץ וכתב, ומה שנופל הוא
ה-GET שמביא את התשובה מ-`script.googleusercontent.com`.

לא תוקן, במכוון. ההתנהגות הנכונה כבר קיימת:

```
POST → doPost כותב שורה → GET ה-echo נופל 404
     → Worker מחזיר 500, ולא מסמן ב-KV
Meta מנסה שוב → existingIds_ מוצא את המזהה → skipped:1, ok:true
     → Worker מחזיר 200, מסמן ב-KV
```

בסוף יש שורה אחת. זו בדיוק הסיבה ש-dedupe קיים גם בצד הגיליון ולא רק ב-KV.

**בבדיקה מלוח הבקרה אין retry.** לכן אחרי לחיצה על Send יש לבדוק את הגיליון
לפני שמסיקים כישלון מ-500 בלוג.
